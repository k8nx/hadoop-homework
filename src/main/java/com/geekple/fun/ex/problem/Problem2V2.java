...
import com.geekple.fun.ex.common.TuplePartitioner;
import com.geekple.fun.ex.common.TupleSampler;
...
public class Problem2V2 extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		job.setJobName("join");
		job.setJarByClass(Problem2V2.class);
		 ...
		 * 6. Projections 작성시 (조인에 사용할 조건을 제외)  alias와 무관하게 field name 이 같아서는 안됨.
		 * 	 아래처럼 dname, ename 으로 분리한 이유.
		...
		TuplePartitioner.useGroupComparator(job.getConfiguration(), true);
		TuplePartitioner.setPartitionFile(job.getConfiguration(), new Path("/geekple/input/cached/_partitions"));
		Sampler<Key, TupleEx> sampler = new TupleSampler.RandomSampler(0.3, 3000, 10);
		TuplePartitioner.writePartitionFile(job, sampler);
		
		URI partitionUri = new URI(TuplePartitioner.getPartitionFile(job.getConfiguration()).toString() + "#_partitions");
		DistributedCache.addCacheFile(partitionUri, job.getConfiguration());
		DistributedCache.createSymlink(job.getConfiguration());
		
		boolean success = job.waitForCompletion(true);
		FileSystem.get(getConf()).delete(new Path("/geekple/input/cached/_partitions"), true);
		return success ? 0 : 1;
	}
	
	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new Problem2V2(), args);
